-- Migration: Schema Update for Sales Intelligence AI
-- Description: Standardize column names and add user_id for multi-user support
-- Date: 2026-05-10

-- Step 1: Create file_metadata table for tracking uploaded files
...
-- Step 2: Create new sales_data table with standardized columns
CREATE TABLE IF NOT EXISTS sales_data_new (
...
-- Step 3: Migrate existing data from old table to new table (if old table exists)
-- Note: This assumes the old table has columns: date, product, quantity, price, revenue, file_id
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'sales_data') THEN
        -- Migrate data with column name mapping
        ...
        -- Rename new table to sales_data
    ...
    ELSE
        ...
        ALTER TABLE sales_data_new RENAME TO sales_data;
        RAISE NOTICE 'New sales_data table created';
    END IF;
END $$;
...
COMMENT ON TABLE sales_data IS 'Stores sales transaction data with standardized column names';
